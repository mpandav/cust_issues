...
func (t *Trigger) Initialize(ctx trigger.InitContext) error {
	...
	for _, handler := range ctx.GetHandlers() {
		...
		tags["Service"] = settings.ServiceName
		tags["Method"] = settings.MethodName
	...
	if t.settings.EnableTLS {
		...
		if t.settings.RootCA != "" {
			rootCA, err := t.decodeCertificate(t.settings.RootCA)
			if err != nil {
				if strings.Contains(err.Error(), "No content found for certificate") {
					t.settings.RootCA = ""
				} else {
					t.Logger.Errorf("Error decoding root CA: %s", err.Error())
					return err
				}
			}
			t.settings.RootCA = string(rootCA)
		}
	}
	return nil
}
...
func (t *Trigger) Start() error {
	...
	if t.settings.EnableTLS {
		...
		// mTLS
		if t.settings.EnableMTLS {
			tlsConfig := &tls.Config{
				Certificates: []tls.Certificate{cert},
			}
			cp := x509.NewCertPool()
			if t.settings.RootCA != "" && !cp.AppendCertsFromPEM([]byte(t.settings.RootCA)) {
...
func (t *Trigger) decodeCertificate(cert string) ([]byte, error) {
	...
	if err == nil {
		t.Logger.Debug("Certificate received as base64 encoded string")
		return decode, nil
	}
