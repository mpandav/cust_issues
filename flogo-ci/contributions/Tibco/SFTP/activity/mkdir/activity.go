...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "SFTP-activity-mkdir"), activityName: "mkdir"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	//mkdir operation
	mkdirOperationOutput, err := flogosftp.MkdirOperation(sc, inputParams, activity.activityName)
	if err != nil {
		...
		if flogosftp.IsConnectionLost(err) || !flogosftp.IsConnectionAlive(sc) {
			...
			if err != nil {
				...
				sc = input.Connection.GetConnection().(*sftp.Client)
				// Retry mkdir operation after reconnection
				mkdirOperationOutput, _ = flogosftp.MkdirOperation(sc, inputParams, activity.activityName)
			}
		} else {
	...
	out, err := coerce.ToString(mkdirOperationOutput)
	if err != nil {
		return false, flogosftp.GetError(flogosftp.DefaultError, activity.activityName, err.Error())
	}
	...
	json.NewEncoder(reqBodyBytes).Encode(mkdirOperationOutput)
