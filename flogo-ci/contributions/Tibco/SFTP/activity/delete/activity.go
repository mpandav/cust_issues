...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "SFTP-activity-delete"), activityName: "delete"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	//delete operation
	err = flogosftp.DeleteOperation(sc, inputParams, activity.activityName, activity.logger)
	if err != nil {
		...
		if flogosftp.IsConnectionLost(err) || !flogosftp.IsConnectionAlive(sc) {
			...
			if err != nil {
				...
				sc = input.Connection.GetConnection().(*sftp.Client)
				// Retry delete operation after reconnection
				_ = flogosftp.DeleteOperation(sc, inputParams, activity.activityName, activity.logger)
			}
		} else {
