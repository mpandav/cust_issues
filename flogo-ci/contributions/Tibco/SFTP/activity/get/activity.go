...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "SFTP-activity-get"), activityName: "get"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	//get operation
	var getOperationOutput *flogosftp.Output
	processData := input.ProcessData
	overwrite := input.Overwrite
	if processData {
		getOperationOutput, err = flogosftp.ProcessDataGetOperation(sc, inputParams, input.Binary, activity.activityName)
	} else {
		getOperationOutput, err = flogosftp.FileTransferGetOperation(sc, inputParams, overwrite, activity.activityName)
	}
	...
	if err != nil {
		...
		if flogosftp.IsConnectionLost(err) || !flogosftp.IsConnectionAlive(sc) {
			...
			if err != nil {
				...
				sc = input.Connection.GetConnection().(*sftp.Client)
				// Retry get operation after reconnection
				if processData {
					getOperationOutput, _ = flogosftp.ProcessDataGetOperation(sc, inputParams, input.Binary, activity.activityName)
				} else {
					getOperationOutput, _ = flogosftp.FileTransferGetOperation(sc, inputParams, overwrite, activity.activityName)
				}
			}
		} else {
	...
	out, err := coerce.ToString(getOperationOutput)
	if err != nil {
		return false, flogosftp.GetError(flogosftp.DefaultError, activity.activityName, err.Error())
	}
	...
	json.NewEncoder(reqBodyBytes).Encode(getOperationOutput)
