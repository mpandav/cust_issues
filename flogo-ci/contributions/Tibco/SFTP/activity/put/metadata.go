...
type Input struct {
	...
	ProcessData bool                   `md:"processdata,required"`
	Binary      bool                   `md:"binary,required"`
	Overwrite   bool                   `md:"overwrite,required"`
...
func (i *Input) FromMap(values map[string]interface{}) error {
	...
	i.ProcessData, err = coerce.ToBool(values["processdata"])
	if err != nil {
		return err
	}

	i.Binary, err = coerce.ToBool(values["binary"])
	if err != nil {
		return err
	}
