...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "SFTP-activity-list"), activityName: "list"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	//list operation
	listOperationOutput, err := flogosftp.ListOperation(sc, inputParams, activity.activityName)
	if err != nil {
		...
		if flogosftp.IsConnectionLost(err) || !flogosftp.IsConnectionAlive(sc) {
			...
			if err != nil {
				...
				sc = input.Connection.GetConnection().(*sftp.Client)
				// Retry list operation after reconnection
				listOperationOutput, _ = flogosftp.ListOperation(sc, inputParams, activity.activityName)
			}
		} else {
	...
	out, err := coerce.ToString(listOperationOutput)
	if err != nil {
		return false, flogosftp.GetError(flogosftp.DefaultError, activity.activityName, err.Error())
	}
	...
	json.NewEncoder(reqBodyBytes).Encode(listOperationOutput)
