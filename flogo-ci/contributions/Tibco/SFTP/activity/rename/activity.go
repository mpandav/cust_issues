...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "SFTP-activity-rename"), activityName: "rename"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	//rename operation
	renameOperationOutput, err := flogosftp.RenameOperation(sc, inputParams, activity.activityName)
	if err != nil {
		...
		if flogosftp.IsConnectionLost(err) || !flogosftp.IsConnectionAlive(sc) {
			...
			if err != nil {
				...
				sc = input.Connection.GetConnection().(*sftp.Client)
				// Retry rename operation after reconnection
				renameOperationOutput, _ = flogosftp.RenameOperation(sc, inputParams, activity.activityName)
			}
		} else {
	...
	out, err := coerce.ToString(renameOperationOutput)
	if err != nil {
		return false, flogosftp.GetError(flogosftp.DefaultError, activity.activityName, err.Error())
	}
	...
	json.NewEncoder(reqBodyBytes).Encode(renameOperationOutput)
