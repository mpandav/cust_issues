...
func TestRenameOperation(t *testing.T) {
	...
	var inputJSON = []byte(`{
		"Old Remote File Name": "test.txt",
		"New Remote File Name": "test1.txt"
	}`)
...
func Execute(t *testing.T, inputParams map[string]interface{}) {
	renameActivity := &MyActivity{logger: log.ChildLogger(log.RootLogger(), "SFTP-rename"), activityName: "rename"}
	//set logging to debug level
	log.SetLogLevel(renameActivity.logger, log.DebugLevel)

	tc := test.NewActivityContext(renameActivity.Metadata())
