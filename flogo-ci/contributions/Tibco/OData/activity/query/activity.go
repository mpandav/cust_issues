...
// ODataQueryActivity is an Activity that is used to invoke a OData service
type ODataQueryActivity struct {
...
func init() {
	_ = activity.Register(&ODataQueryActivity{}, New)
}

// New creates new instance of ODataQueryActivity
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	return &ODataQueryActivity{client: client}, nil
}

// Metadata returns the activity's metadata
func (a *ODataQueryActivity) Metadata() *activity.Metadata {
	return activityMd
}

// Eval implements activity.Activity.Eval - Invokes a OData service endpoint
func (a *ODataQueryActivity) Eval(context activity.Context) (done bool, err error) {
	input := &Input{}
	context.Logger().Info("Executing OData Query Activity")
	...
	resp, err = oDataConnection.SendRequest(client, http.MethodGet, uri, getHeaders(context, queryOpts), nil, "", context.Logger())
	...
	context.Logger().Info("OData query activity response status: ", resp.Status)
	...
	context.Logger().Debugf("OData query response content-type: %s", contentType)
	...
	if err != nil {
		return false, fmt.Errorf("error setting output for ODataQueryActivity [%s]: %s", context.Name(), err.Error())
	}
