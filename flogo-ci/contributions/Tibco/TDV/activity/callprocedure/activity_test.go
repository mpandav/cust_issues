...
import (
	...
	"git.tibco.com/git/product/ipaas/wi-tdv.git/src/app/TDV/connector/connection/connection"
	"git.tibco.com/git/product/ipaas/wi-tdv.git/src/app/github.com/TIBCOSoftware/flogo-lib/core/activity"
	"git.tibco.com/git/product/ipaas/wi-tdv.git/src/app/github.com/TIBCOSoftware/flogo-lib/core/data"
	"git.tibco.com/git/product/ipaas/wi-tdv.git/src/app/github.com/TIBCOSoftware/flogo-lib/logger"
...
var connectionJSON = `{
	 "id" : "TDVTestConnection",
	 "name": "tibco-tdv",
	 "description" : "TDV Test Connection",
	 "title": "TDV Connector",
	 "type": "flogo:connector",
	 "version": "1.0.0",
	 "ref": "https://git.tibco.com/git/product/ipaas/wi-tdv.git/activity/query",
	 "keyfield": "name",
	 "settings": [
		 {
		   "name": "name",
...
func getConnection(t *testing.T) (connection *tdv.Connection, err error) {
	connector, err := getConnector(t, true)
	assert.NotNil(t, connector)

	connection, err = tdv.GetConnection(connector)
	if err != nil {
		t.Errorf("TDV get connection failed %s", err.Error())
		t.Fail()
	}
...
func TestGetConnection(t *testing.T) {
	...
	assert.Nil(t, err)
	if err != nil {
		t.Errorf("TDV Login failed %s", err.Error())
	}
	connection.Logout()
}
...
func TestInvalidGetConnection(t *testing.T) {
	...
	err = connection.Login()
	if err != nil {
		fmt.Printf("TDV Login failed [%s] as expected \n", err.Error())
	}
	assert.Error(t, err)
	connection.Logout()
}
...
func TestBlobs(t *testing.T) {
	...
	var tests = []struct {
		...
		{id: `InsertPngBlob`,
			...
			errorMsg:  `no error`,
			fieldName: "logo",
			fileName:  "../insert/icons/ic-tdv-insert@3x.png",
		},
	}
	conn, err := GetTDVObject()
	if err != nil {
...
func testBlob(t *testing.T, id string, query string, input string, fields string,
	...
	record, ok := outputData.(*tdv.ResultSet)
	if !ok {
...
// GetTDVObject for testing
func GetTDVObject() (connector interface{}, err error) {
