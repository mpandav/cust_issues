...
// MysqlDeleteActivity is the structure for Activity Metadata
type MysqlDeleteActivity struct {
	settings *Settings
}

func init() {
	_ = activity.Register(&MysqlDeleteActivity{}, New)
}

// New constructor for creating a New MySQL activity
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MysqlDeleteActivity{}, nil
}

// Metadata returns MySQL's Query activity's meteadata
func (a *MysqlDeleteActivity) Metadata() *activity.Metadata {
	return activityMd
}

// Eval handles the data processing in the activity
func (a *MysqlDeleteActivity) Eval(context activity.Context) (done bool, err error) {
	logger := context.Logger()
	logger.Info("Executing MySQL Delete Activity")
	actinputs := &Input{}
	...
	// b, _ := json.Marshal(inputParams)
	// fmt.Println(string(b))
	if err != nil {
		return false, fmt.Errorf("Failed to process input arguments, %s", err.Error())
	}

	result, err := sharedmanager.PreparedDelete(actinputs.DeleteStatement, inputParams, logger)
	if err != nil {
		...
		if err == driver.ErrBadConn || err == mysql.ErrInvalidConn || strings.Contains(err.Error(), "connection refused") || strings.Contains(err.Error(), "network is unreachable") ||
			...
			return false, activity.NewRetriableError(fmt.Sprintf("Failed to execute query [%s] on MySQL server due to error - {%s}.", actinputs.DeleteStatement, err.Error()), "mysql-delete-4001", nil)
		}
