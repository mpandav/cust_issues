...
// MysqlInsertActivity is the structure for Activity Metadata
type MysqlInsertActivity struct {
	settings *Settings
}

func init() {
	_ = activity.Register(&MysqlInsertActivity{}, New)
}

// New constructor for creating a New MySQL activity
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MysqlInsertActivity{}, nil
}

// Metadata returns MySQL's Query activity's meteadata
func (a *MysqlInsertActivity) Metadata() *activity.Metadata {
	return activityMd
}

// Eval handles the data processing in the activity
func (a *MysqlInsertActivity) Eval(context activity.Context) (done bool, err error) {
	logger := context.Logger()
	logger.Info("Executing MySQL Insert Activity")
	actinputs := &Input{}
	...
	result, err := sharedmanager.PreparedInsert(actinputs.InsertStatement, mysqlInputs, actinputs.Fields, logger)
	if err != nil {
		...
		if err == driver.ErrBadConn || err == mysql.ErrInvalidConn || strings.Contains(err.Error(), "connection refused") || strings.Contains(err.Error(), "network is unreachable") ||
			...
			return false, activity.NewRetriableError(fmt.Sprintf("Failed to execute query [%s] on MySQL server due to error - {%s}.", actinputs.InsertStatement, err.Error()), "mysql-insert-4001", nil)
		}
