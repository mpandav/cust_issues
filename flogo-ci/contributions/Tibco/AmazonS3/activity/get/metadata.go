...
type Input struct {
	...
	OperationType string                 `md:"operationType,required"` // Select one operation type from Single Object,List Objects,List Buckets
	OutputType    string                 `md:"outputType"`             // Select one output type from File,Text
...
func (i *Input) FromMap(values map[string]interface{}) error {
	...
	i.OutputType, err = coerce.ToString(values["outputType"])
	if err != nil {
		return err
	}
