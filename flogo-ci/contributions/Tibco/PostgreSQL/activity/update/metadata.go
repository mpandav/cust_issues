...
import (
	...
	psqlconnection "github.com/tibco/wi-postgres/src/app/PostgreSQL/connector/connection"
)

// Input Structure
type Input struct {
	Connection   connection.Manager     `md:"Connection,required"`
	UpdateName   string                 `md:"UpdateName"`
...
func (o *Input) FromMap(values map[string]interface{}) error {
	var err error
	o.UpdateName, err = coerce.ToString(values["Updatename"])
	if err != nil {
		return err
	}
