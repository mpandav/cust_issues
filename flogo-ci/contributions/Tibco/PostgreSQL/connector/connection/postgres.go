...
func (p *PgSharedConfigManager) PreparedInsert(queryString string, inputData *Input, fields interface{}, logCache log.Logger) (results *ResultSet, err error) {
	...
	// logCache.Debugf("Executing prepared Insert query %s", queryString)
	// logCache.Debugf("Input arguments data: %v", inputData)
...
func UnmarshalRows(rows *sql.Rows) (results *ResultSet, err error) {
	...
	for rows.Next() {
		...
		for i, b := range cols {
			...
			// Added support
			// WIPGRS-257, WIPGRS-428, WIPGRS-461, WIPGRS-462
			case "XML", "JSON", "JSONB", "":
