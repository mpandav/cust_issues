...
//append the rows from inputData.values to the values clause
func updateInsertQuery(query string, inputData Input, inputArgs []interface{}, paramsarray []string) (string, []interface{}) {
	...
	// get index of ')' of last inserted row in values clause
	...
	for i := 1; i < len(inputData.Values); i++ {
		for _, key := range keys {
			inputArgs = append(inputArgs, inputData.Values[i][key])
			value = fmt.Sprintf("%s$%d,", value, cnt)
			cnt++
		}
		value = value[:len(value)-1]
		preparedValues = fmt.Sprintf("%s,(%s)", preparedValues, value) //preparedValues contains all rows to be appended to values clause
		value = ""
	}
...
func EvaluateQuery(query string, inputData Input) (string, []interface{}, []string, error) {
	...
	if strings.ToLower(strings.Split(prepared, " ")[0]) == "insert" {
