...
var myActivity = &MyActivity{logger: log.ChildLogger(log.RootLogger(), "Snowflake-activity-command"), activityName: "command"}
...
func TestPutCommand(t *testing.T) {
	...
	var inputJSON = []byte(`{
		"fileName": "/home/rgohate-t14/Snowflake/mydata1.csv",
		"internalStage": "@~/Test3",
		"overwrite": false
	}`)
	...
	PreparedAndExecuteCommand(t, command, inputParams)
}

func TestCopyIntoCommandSize(t *testing.T) {
	command := "COPY INTO"
...
func TestCopyIntoCommandPurge(t *testing.T) {
	command := "COPY INTO"
...
func TestCopyIntoCommandTruncateColumns(t *testing.T) {
	command := "COPY INTO"
...
func TestCopyIntoCommandValidation(t *testing.T) {
	command := "COPY INTO"
...
func TestCopyIntoCommandType(t *testing.T) {
	command := "COPY INTO"
...
func TestCopyIntoCommandFileFormat(t *testing.T) {
	command := "COPY INTO"
...
func TestCopyIntoCommandFormatTypeOptions(t *testing.T) {
	command := "COPY INTO"
	...
	var inputJSON = []byte(`{
		...
		"stageName": "@~/Test3",
		"type": "CSV",
		"formatTypeOptions":"{\"field_delimiter\": \",\", \"compression\": \"GZIP\"}",
			"copyOptions": {
				"on_error": "SKIP_FILE"
			  }
		}`)
...
func PreparedAndExecuteCommand(t *testing.T, command string, inputParams map[string]interface{}) {
	//set logging to debug level
	log.SetLogLevel(myActivity.logger, log.DebugLevel)

	tc := test.NewActivityContext(myActivity.Metadata())

	aInput := &Input{Connection: connManager, Command: command, Input: inputParams}
	tc.SetInputObject(aInput)
	ok, err := myActivity.Eval(tc)
	assert.True(t, ok)
	if err != nil {
		t.Errorf("Failed to perform command: %s", err.Error())
		t.Fail()
	}
	...
	assert.Nil(t, err)
	if err != nil {
		t.Errorf("Failed to get output of command: %s", err.Error())
		t.Fail()
	}
}
