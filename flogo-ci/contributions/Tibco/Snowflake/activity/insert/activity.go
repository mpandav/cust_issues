...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "Snowflake-activity-insert"), activityName: "insert"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	//resultSet, err := snowflake.ExecutePreparedInsert(queryString, inputObj, db, ActivityName, log)
	result, err := snowflake.PreparedInsert(query, inputParams, db, activity.activityName, activity.logger)
	if err != nil {
