...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "Snowflake-activity-update"), activityName: "update"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	result, err := snowflake.ExecutePreparedUpdate(query, inputParams, db, activity.activityName, activity.logger)
	if err != nil {
