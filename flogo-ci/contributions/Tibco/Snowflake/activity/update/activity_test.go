...
var myActivity = &MyActivity{logger: log.ChildLogger(log.RootLogger(), "Snowflake-activity-update"), activityName: "update"}
...
var snowflakeOAuthConnectionJSON = []byte(`{
	...
	"authCode": "E63837F1A4AA010A4620D0495616A93E9CDB5B89",
...
func setup() {
	...
	createFLOGOEMPUPDATETable := `create or replace table FLOGOEMP_UPDATE (ID number, NAME string);`
	insertIntoFLOGOEMPUPDATE := `insert into FLOGOEMP_UPDATE values (1, 'tom'), (2, 'chris')`
	db.Exec(createFLOGOEMPUPDATETable)
	db.Exec(insertIntoFLOGOEMPUPDATE)

	createFLOGOSTUDENTUPDATETable := `create or replace table FLOGOSTUDENT_UPDATE (ROLLNUM number, STDNAME string, MARKS number);`
	insertIntoFLOGOSTUDENTUPDATE := `insert into FLOGOSTUDENT_UPDATE values (1,'Tom',23), (2,'Tom',25), (3,'John',20), (100, 'Hero', 100)`
	db.Exec(createFLOGOSTUDENTUPDATETable)
	db.Exec(insertIntoFLOGOSTUDENTUPDATE)

	createVARIANTTable1 := `create table variant_table1 (float1 float, v variant, float2 float);`
	insertVARIANTTable1 := `insert into variant_table1 (float1, v, float2) values (1.23, null, null), (1, null, null);`
	db.Exec(createVARIANTTable1)
	db.Exec(insertVARIANTTable1)
}
...
func TestUpdateSQL(t *testing.T) {
	query := `UPDATE FLOGOEMP_UPDATE SET NAME = ?NAME WHERE ID = ?ID;`
...
func TestUpdateSQLRepeatedParameters(t *testing.T) {
	query := `UPDATE FLOGOSTUDENT_UPDATE SET STDNAME = ?NAME WHERE ROLLNUM = ?input AND MARKS = ?input;`
...
func TestUpdateVariantDatatype1(t *testing.T) {
	query := `update variant_table1 set v = to_variant(float1) where float1 = 1.23;`
...
func TestUpdateVariantDatatype2(t *testing.T) {
	query := `update variant_table1 set float2 = v::float where float1 = 1.23;`
...
func TestUpdateVariantDatatype3(t *testing.T) {
	query := `update variant_table1 set V = '{"key1":"value1"}' where float1 = 1;`
...
func TestUpdateVariantDatatype4(t *testing.T) {
	query := `UPDATE variant_table1 SET V = OBJECT_INSERT(V, 'key1', 'newValue1', TRUE) WHERE V:key1::VARCHAR = 'value1';`
