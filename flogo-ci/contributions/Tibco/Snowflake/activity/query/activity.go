...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "Snowflake-activity-query"), activityName: "query"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	result, err := snowflake.ExecutePreparedQuery(query, inputParams, db, activity.activityName, activity.logger)
	if err != nil {
