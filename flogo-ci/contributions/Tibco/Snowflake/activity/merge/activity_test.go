package merge

import (
	"database/sql"
	"encoding/json"
	"fmt"
	"os"
	"testing"

	"github.com/project-flogo/core/activity"
	"github.com/project-flogo/core/support/connection"
	"github.com/project-flogo/core/support/log"
	"github.com/project-flogo/core/support/test"
	"github.com/stretchr/testify/assert"

	snowflakedb "github.com/tibco/wi-snowflake/src/app/Snowflake/connector/connection"
)

var db *sql.DB
var connManager connection.Manager
var myActivity = &MyActivity{logger: log.ChildLogger(log.RootLogger(), "Snowflake-activity-merge"), activityName: "merge"}

var snowflakeBasicAuthConnectionJSON = []byte(`{
	"name": "snowflakeBasic",
	"description": "",
	"account": "fqb58399.us-east-1",
	"warehouse": "COMPUTE_WH",
	"database": "ESECONNECTOR",
	"schema": "ESESCHEMA",
	"authType": "Basic Authentication",
	"user": "vcastell",
	"password": "L!tmein1now4",
	"role": "SYSADMIN",
	"loginTimeout": 10
}`)

/*
   We are using a trial account for testing purpose.
   When the trial account expires, we would need to create a new account and update below two connection json.
*/

/*
   The Refresh Token validity is 90 days.
   After it is expired, we need to update below json with new refresh token and access token.
*/

/*
   ==> Execute below sql with ACCOUNTADMIN role to create an integration object.

   create security integration test_integration_object
   type = oauth
   enabled = true
   oauth_client = CUSTOM
   oauth_client_type = 'CONFIDENTIAL'
   oauth_redirect_uri = 'https://oauthdebugger.com/debug'
   oauth_issue_refresh_tokens = true
   oauth_refresh_token_validity = 86400
   OAUTH_ALLOW_NON_TLS_REDIRECT_URI = false;

   ==> Execute below sql with ACCOUNTADMIN role to get the cliend id and client secret.

   select SYSTEM$SHOW_OAUTH_CLIENT_SECRETS('TEST_INTEGRATION_OBJECT')
*/

/*
Worksheet - https://app.snowflake.com/us-east-1/fqb58399/w5AGTRnNIsTA#query
Authorization Code: Kindly copy paste below URL in the browser to get authorization code and login credentials(vcastell/L!tmein1now4):
https://fqb58399.us-east-1.snowflakecomputing.com/oauth/authorize?response_type=code&client_id=3T447dkBuaOUjDrMETzmppm8cW0%3D&scope=refresh_token&redirect_uri=https://localhost.com
*/
var snowflakeOAuthConnectionJSON = []byte(`{
	"name": "snowflakeOAuth",
	"description": "",
	"account": "fqb58399.us-east-1",
	"warehouse": "COMPUTE_WH",
	"database": "ESECONNECTOR",
	"schema": "ESESCHEMA",
	"authType": "OAuth",
	"user": "vcastell",
	"password": "L!tmein1now4",
	"clientId": "3T447dkBuaOUjDrMETzmppm8cW0=",
	"clientSecret": "uqN8N4Eh0qSYze3a8CC+rLGf7oWUoZhqOnlEZ+t4Xng=",
	"authCode": "8C644D1FDEC85C488785D9C76221DD9924A56579",
	"redirectURI" : "https://localhost.com",
	"accessToken": "SECRET:",
    "accessTokenExpiry": 1660143392631,
    "refreshToken": "SECRET:",
    "refreshTokenExpiry": 1660412792631,
    "role": "",
    "loginTimeout": 10,
    "codeCheck": "2B9E1F32AA5407F3BEF4805E16222027F7FA01E4"
}`)

var snowflakeOktaOAuthConnectionJSON = []byte(`{
	"name": "snowflakeOktaOAuth",
	"description": "",
	"account": "fqb58399.us-east-1",
	"warehouse": "COMPUTE_WH",
	"database": "ESECONNECTOR",
	"schema": "ESESCHEMA",
	"authType": "OAuth",
	"provider" : "Okta with PKCE",
	"clientId": "0oaa3k25ryZIiddFI5d7",
	"authCode": "mvny4wzfOu_UTF3deuEthmGJ1fP99_Vc3y0j36US4c8",
	"scope": "session:role:sysadmin offline_access",
    "oktaCodeVerifier": "M25iVXpKU3puUjFaYWg3T1NDTDQtcW1ROUY5YXlwalNoc0hhakxifmZHag",
    "oktaCodeChallenge": "qjrzSW9gMiUgpUvqgEPE4_-8swvyCtfOVvg55o5S_es",
	"redirectURI" : "https://integration.local.cic2.pro",
	"oktaAccessToken": "SECRET:",
    "oktaAccessTokenExpiry": 1687944041000,
	"oktaRefreshToken": "4Mwvacpw7ZQar37nCxJktb9Jlxel1cZUXGt5IVzCB78",
	"loginTimeout": 10,
	"oktaTokenEndpoint": "https://dev-91275340.okta.com/oauth2/ausa3k6f8ifq4xY5x5d7/v1/token",
    "codeCheck": "xO3486VQiCT3t_14AUKrs7DFQlg54o933gTXZlhNO1w"
}`)

func TestRegister(t *testing.T) {
	ref := activity.GetRef(&MyActivity{})
	act := activity.Get(ref)

	assert.NotNil(t, act)
}

func setup() {
	conn := make(map[string]interface{})

	//comment and uncomment based on type of connection JSON you want to use - Basic or OAuth
	err := json.Unmarshal([]byte(snowflakeBasicAuthConnectionJSON), &conn)
	//err := json.Unmarshal([]byte(snowflakeOAuthConnectionJSON), &conn)

	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}

	sdb := &snowflakedb.SnowflakeFactory{}
	connManager, err = sdb.NewManager(conn)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}

	db = connManager.GetConnection().(*sql.DB)

	createSOURCETable1 := `create or replace table source_table1 (ID INTEGER, description VARCHAR);`
	insertIntoSOURCETable1 := `insert into source_table1 (ID, description) VALUES (10, 'abcNew');`
	db.Exec(createSOURCETable1)
	db.Exec(insertIntoSOURCETable1)

	createTARGETTable1 := `create or replace table target_table1 (ID INTEGER, description VARCHAR);`
	insertIntoTABLETable1 := `insert into target_table1 (ID, description) VALUES (10, 'abcOld');`
	db.Exec(createTARGETTable1)
	db.Exec(insertIntoTABLETable1)

	createSOURCETable2 := `create or replace table source_table2 (ID INTEGER, description VARCHAR);`
	insertIntoSOURCETable2 := `insert into source_table2 (ID, description) VALUES (10, 'abcNew');`
	insertIntoSOURCETable21 := `INSERT INTO source_table2 (ID, description) VALUES (50, 'temp'), (50, 'temp1');`
	db.Exec(createSOURCETable2)
	db.Exec(insertIntoSOURCETable2)
	db.Exec(insertIntoSOURCETable21)

	createTARGETTable2 := `create or replace table target_table2 (ID INTEGER, description VARCHAR);`
	insertIntoTABLETable2 := `insert into target_table2 (ID, description) VALUES (10, 'abcOld');`
	db.Exec(createTARGETTable2)
	db.Exec(insertIntoTABLETable2)

	createSOURCETable3 := `create or replace table source_table3 (ID INTEGER, description VARCHAR);`
	insertIntoSOURCETable3 := `insert into source_table3 (ID, description) VALUES (10, 'abcNew'), (20, 'same');`
	insertIntoSOURCETable31 := `INSERT INTO source_table3 (ID, description) VALUES (50, 'temp'), (50, 'temp1');`
	db.Exec(createSOURCETable3)
	db.Exec(insertIntoSOURCETable3)
	db.Exec(insertIntoSOURCETable31)

	createTARGETTable3 := `create or replace table target_table3 (ID INTEGER, description VARCHAR);`
	insertIntoTABLETable3 := `insert into target_table3 (ID, description) VALUES (10, 'abcOld'), (20, 'same');`
	db.Exec(createTARGETTable3)
	db.Exec(insertIntoTABLETable3)
}

func shutdown() {
	if db != nil {
		db.Close()
	}
}

func TestMain(m *testing.M) {
	//set logging to debug level
	log.SetLogLevel(myActivity.logger, log.DebugLevel)

	setup()
	code := m.Run()
	shutdown()
	os.Exit(code)
}

func TestMergeSQLUpdate(t *testing.T) {
	query := `MERGE INTO target_table1 USING source_table1 
	ON target_table1.id = source_table1.id 
	WHEN MATCHED THEN 
	UPDATE SET target_table1.description = source_table1.description;`

	inputParams := make(map[string]interface{})
	var inputJSON = []byte(`{
        "parameters": {
        }
    }`)
	err := json.Unmarshal(inputJSON, &inputParams)
	assert.Nil(t, err)

	PreparedAndExecuteSQL(t, query, inputParams)
}

func TestMergeSQLUpdateInsert(t *testing.T) {
	query := `MERGE INTO target_table2 USING source_table2 
	ON target_table2.id = source_table2.id 
	WHEN MATCHED THEN 
	UPDATE SET target_table2.description = source_table2.description 
	WHEN NOT MATCHED THEN 
	INSERT (ID, description) VALUES (source_table2.id, source_table2.description);`

	inputParams := make(map[string]interface{})
	var inputJSON = []byte(`{
        "parameters": {
        }
    }`)
	err := json.Unmarshal(inputJSON, &inputParams)
	assert.Nil(t, err)

	PreparedAndExecuteSQL(t, query, inputParams)
}

func TestMergeSQLDeleteInsert(t *testing.T) {
	query := `MERGE INTO target_table3 USING source_table3 
	ON target_table3.id = source_table3.id AND target_table3.id = ?id
	WHEN MATCHED THEN 
	DELETE
	WHEN NOT MATCHED THEN 
	INSERT (ID, description) VALUES (source_table3.id, source_table3.description);`

	inputParams := make(map[string]interface{})
	var inputJSON = []byte(`{
        "parameters": {
            "id": 10
        }
    }`)
	err := json.Unmarshal(inputJSON, &inputParams)
	assert.Nil(t, err)

	PreparedAndExecuteSQL(t, query, inputParams)
}

func PreparedAndExecuteSQL(t *testing.T, query string, inputParams map[string]interface{}) {
	tc := test.NewActivityContext(myActivity.Metadata())

	aInput := &Input{Connection: connManager, Query: query, Input: inputParams}
	tc.SetInputObject(aInput)
	ok, _ := myActivity.Eval(tc)
	assert.True(t, ok)
	aOutput := &Output{}
	err := tc.GetOutputObject(aOutput)
	assert.Nil(t, err)

	if err != nil {
		t.Errorf("Could not execute prepared query %s", query)
		t.Fail()
	}
}
