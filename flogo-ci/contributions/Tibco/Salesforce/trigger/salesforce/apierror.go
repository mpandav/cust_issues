...
func (e ApiErrors) DuplicateTopicError() bool {
	for _, err := range e {
		if err.DuplicateTopicError() {
			return true
		}
	}
	return false
...
func (e ApiError) DuplicateTopicError() bool {

	if e.ErrorCode != "" && strings.EqualFold(e.ErrorCode, "DUPLICATE_VALUE") {
		return true
	}

	if e.Message != "" && strings.EqualFold(e.Message, "A PushTopic with this name already exists. Please choose a different name.") {
		return true
	}
