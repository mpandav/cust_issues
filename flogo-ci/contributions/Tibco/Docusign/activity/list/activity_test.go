...
func TestListDocuments(t *testing.T) {
	...
	listActivity := &ListDocumentsActivity{}
	activityContext := test.NewActivityContext(listActivity.Metadata())
	...
	_, err = listActivity.Eval(activityContext)
	assert.Nil(t, err)
	if err != nil {
		t.Error("could not fetch list")
		t.Fail()
	} else {
		output := activityContext.GetOutput("Output")
