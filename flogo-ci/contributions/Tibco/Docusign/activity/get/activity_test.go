...
func TestRetrieveDocuments(t *testing.T) {
	connectionJSON := `{
		"name":"docusignTest",
		"ref":"#docusign",
		"id":"docusignTest",
	...
	retrieveActivity := &RetrieveDocumentActivity{}
	activityContext := test.NewActivityContext(retrieveActivity.Metadata())
	...
	// activityContext.SetInput("getAllDocuments", true)
	activityContext.SetInput("getAllDocuments", false)
	activityContext.SetInput("documentId", "")

	// activityContext.SetOutput("outputType", "ZIP")
	// activityContext.SetOutput("outputType", "PDF")

	_, err = retrieveActivity.Eval(activityContext)
	assert.Nil(t, err)
	if err != nil {
		t.Error("could not fetch list")
		t.Fail()
	} else {
		output := activityContext.GetOutput("fileType")
		fmt.Println(output)
		assert.NotNil(t, output)
		// assert.Equal(t, "application/zip", output)
		assert.Equal(t, "application/pdf", output)
		output = activityContext.GetOutput("fileContent")
		assert.NotNil(t, output)
