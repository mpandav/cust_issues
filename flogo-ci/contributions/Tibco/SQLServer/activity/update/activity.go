...
// var logCache = log.ChildLogger(log.RootLogger(), "sqlserver-update")

//Eval for Update Activity
func (a *Activity) Eval(context activity.Context) (done bool, err error) {
	logger := context.Logger()
	logger.Info("Executing SQLServer Update Activity")
	...
	result, err := sharedmanager.PreparedUpdate(query, inputParams, input.Fields, logger, queryTimeout)
	if err != nil {
		...
		if err == driver.ErrBadConn || strings.Contains(lowerErr, "connection refused") || strings.Contains(lowerErr, "network is unreachable") ||
			...
			return false, activity.NewRetriableError(fmt.Sprintf("Failed to execute query [%s] on SQLServer server due to error - {%s}.", query, err.Error()), "sqlserver-update-4001", nil)
		}
		return false, fmt.Errorf("Failed to execute SQL Query, %s", err.Error())
	}
	//json schema validation
