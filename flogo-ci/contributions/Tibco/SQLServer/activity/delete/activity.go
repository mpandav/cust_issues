...
//Package delete provides database query execution implementation for
...
// Metadata  returns SqlServer's Delete activity's meteadata
func (*Activity) Metadata() *activity.Metadata {
	return activityMd
}

// var logCache = log.ChildLogger(log.RootLogger(), "sqlserver-delete")

//Eval for Delete Activity
func (a *Activity) Eval(context activity.Context) (done bool, err error) {
	logger := context.Logger()
	logger.Info("Executing SqlServer Delete Activity")
	...
	result, err := sharedmanager.PreparedDelete(query, inputParams, logger, queryTimeout)
	if err != nil {
		...
		if err == driver.ErrBadConn || strings.Contains(lowerErr, "connection refused") || strings.Contains(lowerErr, "network is unreachable") ||
			...
			return false, activity.NewRetriableError(fmt.Sprintf("Failed to execute query [%s] on SQLServer server due to error - {%s}.", query, err.Error()), "sqlserver-delete-4001", nil)
		}
		return false, fmt.Errorf("Failed to execute SQL Query, %s", err.Error())
	}
	//json schema validation
