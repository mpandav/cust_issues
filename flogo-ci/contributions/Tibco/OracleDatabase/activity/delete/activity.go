...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "OracleDatabase-delete"), activityName: "delete"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	if !strings.HasPrefix(strings.ToUpper(query), "DELETE") {
		return false, helper.GetError(helper.FailedExecuteSQL, activity.activityName, errors.New("Not a valid SQL Delete statement"))
	}
