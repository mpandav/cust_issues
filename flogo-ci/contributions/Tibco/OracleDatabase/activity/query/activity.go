...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "OracleDatabase-query"), activityName: "query"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	result, err := helper.PreparedQuery(db, query, inputParams, activity.logger)
	if err != nil {
