...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "File-activity-remove"), activityName: "remove"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	removeRecursive := input.RemoveRecursive

	deleteFileOutput, err := flogofile.RemoveFile(inputParams, removeRecursive, activity.activityName)
	if err != nil {
		return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
	}

	//Logging output at debug level
	out, err := coerce.ToString(deleteFileOutput)
	if err != nil {
		return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
	}
	...
	json.NewEncoder(reqBodyBytes).Encode(deleteFileOutput)
