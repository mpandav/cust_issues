...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "File-activity-write"), activityName: "write"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	writeAs := input.WriteAs
	createNonExistingDir := input.CreateNonExistingDir
	compress := input.Compress

	var writeFileOutput *flogofile.Output
	if writeAs == "Text" {
		writeFileOutput, err = flogofile.WriteTextFile(inputParams, createNonExistingDir, compress, activity.activityName)
		if err != nil {
			return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
		}
	} else {
		writeFileOutput, err = flogofile.WriteBinaryFile(inputParams, createNonExistingDir, compress, activity.activityName)
		if err != nil {
			return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
		}
	}
	...
	out, err := coerce.ToString(writeFileOutput)
	if err != nil {
		return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
	}
	...
	json.NewEncoder(reqBodyBytes).Encode(writeFileOutput)
