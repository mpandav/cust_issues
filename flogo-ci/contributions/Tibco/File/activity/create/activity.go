...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "File-activity-create"), activityName: "create"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	var createFileOutput *flogofile.Output
	if isDir {
		createFileOutput, err = flogofile.CreateDir(inputParams, createNonExistingDir, activity.activityName)
		if err != nil {
			return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
		}
	} else {
		createFileOutput, err = flogofile.CreateFile(inputParams, createNonExistingDir, activity.activityName)
		if err != nil {
			return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
		}
	}
	...
	out, err := coerce.ToString(createFileOutput)
	if err != nil {
		return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
	}
	...
	json.NewEncoder(reqBodyBytes).Encode(createFileOutput)
