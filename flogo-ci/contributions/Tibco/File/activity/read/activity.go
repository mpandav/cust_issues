...
func New(ctx activity.InitContext) (activity.Activity, error) {
	return &MyActivity{logger: log.ChildLogger(ctx.Logger(), "File-activity-read"), activityName: "read"}, nil
}
...
func (activity *MyActivity) Eval(context activity.Context) (done bool, err error) {
	...
	var readFileOutput *flogofile.ReadFileOutput
	if readAs == "Text" {
		readFileOutput, err = flogofile.ReadTextFile(inputParams, compress, activity.activityName)
		if err != nil {
			return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
		}
	} else {
		readFileOutput, err = flogofile.ReadBinaryFile(inputParams, compress, activity.activityName)
		if err != nil {
			return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
		}
	}
	...
	out, err := coerce.ToString(readFileOutput)
	if err != nil {
		return false, flogofile.GetError(flogofile.DefaultError, activity.activityName, err.Error())
	}
	...
	json.NewEncoder(reqBodyBytes).Encode(readFileOutput)
