...
func SQLGetConnectAttr(ConnectionHandle SQLHDBC, Attribute SQLINTEGER, ValuePtr SQLPOINTER, BufferLength SQLINTEGER, StringLengthPtr *SQLINTEGER)(ret SQLRETURN)  {
	r := C.SQLGetConnectAttr(C.SQLHDBC(ConnectionHandle), C.SQLINTEGER(Attribute), C.SQLPOINTER(ValuePtr), C.SQLINTEGER(BufferLength), (*C.SQLINTEGER)(StringLengthPtr))
	return SQLRETURN(r)
}
...
func SQLProcedureColumns ( statementHandle SQLHSTMT,
    CatalogName *SQLCHAR, NameLength1 SQLSMALLINT,
    SchemaName *SQLCHAR, NameLength2 SQLSMALLINT,
    ProcedureName *SQLCHAR, NameLength3 SQLSMALLINT,
    ColumnName *SQLCHAR, NameLength4 SQLSMALLINT) (ret SQLRETURN) {
        r := C.SQLProcedureColumns (
            C.SQLHSTMT(statementHandle),
            (*C.SQLCHAR)(CatalogName), C.SQLSMALLINT(NameLength1),
            (*C.SQLCHAR)(SchemaName), C.SQLSMALLINT(NameLength2),
            (*C.SQLCHAR)(ProcedureName), C.SQLSMALLINT(NameLength3),
            (*C.SQLCHAR)(ColumnName), C.SQLSMALLINT(NameLength4))
        return SQLRETURN(r)
    }
...
func SQLColAttribute(statementHandle SQLHSTMT, columnNumber SQLUSMALLINT, fieldIdentifier SQLUSMALLINT, characterAttribute SQLPOINTER, bufferLength SQLSMALLINT, stringLength *SQLSMALLINT, numericAttribute *SQLLEN) (ret SQLRETURN) {
	r := C.SQLColAttribute(C.SQLHSTMT(statementHandle), C.SQLUSMALLINT(columnNumber), C.SQLUSMALLINT(fieldIdentifier), C.SQLPOINTER(characterAttribute), C.SQLSMALLINT(bufferLength), (*C.SQLSMALLINT)(stringLength), (*C.SQLLEN)(numericAttribute))
	return SQLRETURN(r)
}
...
func SQLExecDirect(statementHandle SQLHSTMT, statementText *SQLCHAR, textLength SQLINTEGER) (ret SQLRETURN) {
...
func SQLDrivers(environmentHandle SQLHENV, direction SQLUSMALLINT, driverDescription *SQLCHAR, bufferLength1 SQLSMALLINT, descriptionLengthPtr *SQLSMALLINT, driverAttributes *SQLCHAR , bufferLength2 SQLSMALLINT, attributesLengthPtr *SQLSMALLINT) (ret SQLRETURN) {
	r := C.SQLDrivers( C.SQLHENV(environmentHandle), C.SQLUSMALLINT(direction),(*C.SQLCHAR)(unsafe.Pointer(driverDescription)), C.SQLSMALLINT(bufferLength1) , (*C.SQLSMALLINT)(descriptionLengthPtr), (*C.SQLCHAR)(unsafe.Pointer(driverAttributes)), C.SQLSMALLINT(bufferLength2), (*C.SQLSMALLINT)(attributesLengthPtr))
	return SQLRETURN(r)
}