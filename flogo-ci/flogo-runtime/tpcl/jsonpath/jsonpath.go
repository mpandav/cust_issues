	...
	for idx, x := range query {
		...
		if token == "." {
			...
			if strings.Contains(token, "[") {
				...
				arrayStartCount := strings.Count(token, "[")
				arrayCloseCount := strings.Count(token, "]")

				if x == ']' && !strings.HasSuffix(token, "\\]") {
					if arrayStartCount == arrayCloseCount {
						if token[0] == '.' {
	...
	if strings.Contains(filter, "[") && strings.Contains(filter, "]") {
		...
		case reflect.Slice:
			for i := 0; i < reflect.ValueOf(obj).Len(); i++ {
				...
				tempV, err := JsonPathLookup(tmp, filter)
				if err != nil {
					return nil, err
				}
				if tempV != nil && len(tempV.([]interface{})) > 0 {
					res = append(res, tmp)
				}
			}
		case reflect.Map:
			var err error
			obj, err = JsonPathLookup(obj, filter)
			if err != nil {
				return nil, err
			}
		}
